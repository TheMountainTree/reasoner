...
# RGB matrix : [ 1.0599465578241038e+03, 0., 9.5488326677588441e+02, 0.,
#        1.0539326808799726e+03, 5.2373858291060583e+02, 0., 0., 1. ]
# 3. 运行检测与分割
image_path = "/home/frank/workspace/Picture_Capture/data/camera2/camera2_color_20260116_140559.png"

# Register folder & directories
# 当前脚本所在目录：~/workspace/reasoner/reasoner
...
# detection_results = open_vocab_per_class_detection(
#     florence2_model,
#     florence2_processor,
#     sam2_predictor,
#     image_path,
...
# detection_results = open_vocabulary_detection_and_segmentation(
#     florence2_model,
#     florence2_processor,
#     sam2_predictor,
#     image_path,
...
detection_results = merged_open_vocabulary_detection(
    florence2_model,
    florence2_processor,
    sam2_predictor,
    image_path,
...
# ================== 1. 读取 RGB / mask ==================
rgb_image = detection_results["image"]          # (720, 1280, 3)
masks_rgb = detection_results["masks"]          # (N, 720, 1280)
class_names = detection_results["class_names"]

rgb_h, rgb_w = rgb_image.shape[:2]

# ================== 2. 读取 depth（单位：米） ==================
depth_path = "/home/frank/workspace/Picture_Capture/data/camera2/camera2_depth_20260116_140559.png"
depth_aligned = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)
assert depth_aligned is not None, "Failed to load depth image"

if depth_aligned.dtype == np.uint16:
    depth_aligned = depth_aligned.astype(np.float32) / 1000.0
else:
    depth_aligned = depth_aligned.astype(np.float32)

depth_h, depth_w = depth_aligned.shape[:2]
...
# ================== 4. 输出目录 ==================
pcd_output_dir = os.path.join(OUTPUT_DIR, "pcd_seg")
...
for i, (mask_rgb, cls_name) in enumerate(zip(masks_rgb, class_names)):

    # ---- mask: RGB → Depth（最近邻）----
    mask_depth = cv2.resize(
        mask_rgb.astype(np.uint8),
        (depth_w, depth_h),
        interpolation=cv2.INTER_NEAREST
    ).astype(bool)

    valid_mask = np.logical_and(mask_depth, depth_aligned > 0)
    if not np.any(valid_mask):
        print(f"[Skip] {cls_name}_{i}: empty mask")
        continue
    ...
    colors = rgb_image[v_rgb, u_rgb].astype(np.float32) / 255.0
    ...
    if len(pcd.points) < 50:
        print(f"[Skip] {cls_name}_{i}: too few points")
        continue

    pcd_path = os.path.join(pcd_output_dir, f"{cls_name}_{i}.pcd")
    o3d.io.write_point_cloud(pcd_path, pcd)
    print(f"[Saved] {pcd_path}  points={len(pcd.points)}")

# ================== 全深度点云（无 mask） ==================
v_all, u_all = np.where(depth_aligned > 0)
z_all = depth_aligned[v_all, u_all]
...
# 颜色（同样映射回 RGB）
u_rgb_all = np.clip((u_all * scale_x).astype(np.int32), 0, rgb_w - 1)
v_rgb_all = np.clip((v_all * scale_y).astype(np.int32), 0, rgb_h - 1)
colors_all = rgb_image[v_rgb_all, u_rgb_all].astype(np.float32) / 255.0

pcd_full = o3d.geometry.PointCloud()
pcd_full.points = o3d.utility.Vector3dVector(points_all)
pcd_full.colors = o3d.utility.Vector3dVector(colors_all)

R = pcd_full.get_rotation_matrix_from_xyz((np.pi, 0, 0))
pcd_full.rotate(R, center=(0, 0, 0))

o3d.io.write_point_cloud(
    os.path.join(pcd_output_dir, "full_depth.pcd"),
    pcd_full
)

print(f"[Saved] full_depth.pcd points={len(pcd_full.points)}")
