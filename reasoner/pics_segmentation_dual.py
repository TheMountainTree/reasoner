...
def get_fitness(pcd1, pcd2, max_dist=0.03):
    """
    计算两个点云的重合度 (Fitness)
...
def clustering_objects(object_list, 
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            elif obb_d < obb_dist_thr:
                is_connected = True
                # print(f"    Edge {i}-{j} ({name1}): OBB Dist Pass ({obb_d:.3f})")
            elif fitness > fitness_thr:
                is_connected = True
...
T_depth2color_cam2 = np.array([
    [ 1.000,  0.008,  0.001,  0.032],
    [-0.008,  0.994, -0.105,  0.001],
    [-0.002,  0.105,  0.995, -0.002],
    [ 0.000,  0.000,  0.000,  1.000]
])
...
# 路径设置
image_path2 = "/home/frank/workspace/Picture_Capture/data/camera2/camera2_color_20260119_141509.png"
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "output_dual")
os.makedirs(OUTPUT_DIR, exist_ok=True)
...
t_step_start = time.time()
print(f"\n====== [2/7] Loading Depth Images ======")
# 注意：深度图单位一般是毫米，除此之外还需要除以1000变为米
d1 = cv2.imread(depth_path1, cv2.IMREAD_UNCHANGED).astype(np.float32) / 1000.0
d2 = cv2.imread(depth_path2, cv2.IMREAD_UNCHANGED).astype(np.float32) / 1000.0
print(f"-- Depth Load Time: {time.time() - t_step_start:.4f}s")
t_step_start = time.time()
print(f"\n====== [3/7] Reconstructing Instance Point Clouds ======")
# 原始实例列表 (全部统一到 Camera 2 坐标系)
raw_objs1 = build_object_list(det_res1, d1, camera1_intrinsics, T_depth2color=T_depth2color_cam1, transform=T_calib, suffix="c1")
raw_objs2 = build_object_list(det_res2, d2, camera2_intrinsics, T_depth2color=T_depth2color_cam2, transform=None, suffix="c2")
print(f"-- Raw Instances: Cam1={len(raw_objs1)}, Cam2={len(raw_objs2)}")
...
clean_objs2 = clustering_objects(raw_objs2, 
                                    dist_thr=INTRA_DIST_THR, 
                                    vol_sim_thr=0.0,
                                    fitness_thr=INTRA_FITNESS_THR,
                                    obb_dist_thr=INTRA_OBB_THR,
...
clean_objs2 = clean_objs2 if clean_objs2 is not None else []

print(f"-- After Intra-Merge: Cam1={len(clean_objs1)}, Cam2={len(clean_objs2)}")
...
all_candidates = clean_objs1 + clean_objs2
final_objects = clustering_objects(all_candidates, 
                                    dist_thr=INTER_DIST_THR, 
                                    vol_sim_thr=INTER_VOL_THR, 
                                    fitness_thr=INTER_FITNESS_THR,
                                    obb_dist_thr=INTER_OBB_THR,
                                    use_volume=True, 
                                    min_size_ratio=0.2)
...
print(f"--------------------------------------------------")
print(f"-- Global Merge Time: {time.time() - t_step_start:.4f}s")
t_step_start = time.time()
print(f"\n====== [6/7] Reconstructing Full Scene Point Clouds ======")
# 单独构建 Camera 1 全景
pcd_full1 = reconstruct_pcd_from_depth(det_res1["image"], d1, camera1_intrinsics, mask=None, T_depth2color=T_depth2color_cam1)
# 单独构建 Camera 2 全景
pcd_full2 = reconstruct_pcd_from_depth(det_res2["image"], d2, camera2_intrinsics, mask=None, T_depth2color=T_depth2color_cam2)
merged_full = None
if pcd_full1 is not None:
    pcd_full1.transform(T_calib)  # 统一对齐到 Cam2
    if pcd_full2 is not None:
...
if pcd_full2 is not None: pcd_full2 = pcd_full2.voxel_down_sample(VOXEL_FULL)
if merged_full is not None: merged_full = merged_full.voxel_down_sample(VOXEL_FULL)
...
if merged_full is not None and len(merged_full.points) > 0:
    merged_save = o3d.geometry.PointCloud(merged_full)
    # merged_save.rotate(flip_R, center=(0, 0, 0))
    o3d.io.write_point_cloud(os.path.join(pcd_seg_dir, "merged_full_scene.pcd"), merged_save)
# 3. [新增] 保存单相机全景（已带颜色、已标定对齐）
...
if pcd_full2 is not None and len(pcd_full2.points) > 0:
    p2_save = o3d.geometry.PointCloud(pcd_full2)
    # p2_save.rotate(flip_R, center=(0, 0, 0))
    o3d.io.write_point_cloud(os.path.join(pcd_seg_dir, "cam2_full_scene_aligned.pcd"), p2_save)
