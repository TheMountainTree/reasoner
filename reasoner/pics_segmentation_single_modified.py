...
def mask_iou_and_ioa(mask1: np.ndarray, mask2: np.ndarray):
    """
    计算两个掩码的 IoU (Intersection over Union) 和 IoA (Intersection over Area of the smaller one)
    返回: (iou, ioa)
    """
    ...
    min_area = min(area1, area2) + 1e-9
    
    iou = float(intersection) / float(union)
    ioa = float(intersection) / float(min_area)
    return iou, ioa

def dedup_det_res_by_mask_iou(det_res, iou_thr=0.90, ioa_thr=0.80, prefer="score_then_area"):
    """
    对检测结果做去重
    ioa_thr: Intersection over Area 阈值，用于处理包含关系（如瓶盖被瓶子包含），默认 0.8
    """
    if det_res is None:
        return None
    ...
    # 排序逻辑优化：
    # 如果 prefer score，则先按 score 降序；若 score 相同，则按 area 降序（大物体优先）
    # 这样能保证外层的 i 总是优于内层的 j (或面积更大)，在 IoA 判断时保留大的删小的
    if scores is not None and prefer.startswith("score"):
        # lexsort 是按最后一个键主排序。这里主键是 -scores，次键是 -areas
    ...
    for _i in range(n):
        ...
        for _j in range(_i + 1, n):
            ...
            iou, ioa = mask_iou_and_ioa(masks[i], masks[j])
            
            # 如果 IoU 很高，说明几乎重叠 -> 删
            # 如果 IoA 很高，说明 j 几乎完全在 i 内部 (因为 i 排序在前，通常面积更大或分数更高) -> 删
...
def grounded_sam2_detection(
    ...
    支持 <CAPTION_TO_PHRASE_GROUNDING> 模式，通过 object_specs 自动构造详细 prompt
    """
    # 如果是 OD 模式，text_input 应为空；如果是 Caption 模式，text_input 可以传入或由 specs 生成
    if task_prompt == "<OD>":
        assert text_input is None, "Text input should be None when calling dense region caption pipeline."

    # 构造 Prompt (如果提供了 object_specs)
    label_map = {}
    if task_prompt == "<CAPTION_TO_PHRASE_GROUNDING>" and object_specs is not None:
        phrases = []
        print(f"\n[DEBUG] --- Processing JSON Object Specs ---")
        for noun, specs_list in object_specs.items():
            noun_natural = noun.replace("_", " ")
            for specs in specs_list:
                # 打印读取到的 JSON 内容
                raw_attrs = specs.get('attributes', [])
                raw_spatial = specs.get('spatial', [])
                print(f"  > Target: '{noun_natural}'")
                print(f"    - Attributes: {raw_attrs}")
                print(f"    - Spatial:    {raw_spatial}")

                # 构造: "{attributes} {noun} {spatial}"
                parts = []
                if 'attributes' in specs:
                    parts.extend(specs['attributes'])
                
                parts.append(noun_natural)
                
                if 'spatial' in specs:
                    # 简单拼接 "on the left" 等
                    for sp in specs['spatial']:
                         parts.append(f"on the {sp}")
                
                phrase = " ".join(parts).strip()
                # 移除多余空格
                phrase = " ".join(phrase.split())
                phrases.append(phrase)
                label_map[phrase] = noun # 记录映射关系: "red bottle" -> "bottle"
        
        # 覆盖传入的 text_input
        text_input = ". ".join(phrases)
        if not text_input.endswith("."):
            text_input += "."
        
        print(f"[GroundedSAM2] Constructed Prompt: {text_input}")
    ...
    if "bboxes" not in results or len(results["bboxes"]) == 0:
        ...
        return {
            'image': image_np,
            'boxes': np.array([]),
            'masks': np.zeros((0, image_np.shape[0], image_np.shape[1])),
            'class_names': [],
            'class_ids': np.array([]),
            'scores': np.array([])
        }
    ...
    # 映射 Label (Phrase -> Noun)
    class_names = []
    if label_map:
        for lbl in raw_labels:
            # 尝试直接匹配
            if lbl in label_map:
                class_names.append(label_map[lbl])
            else:
                # 模糊匹配 (Looking for known keys inside the label)
                matched_noun = lbl
                for phrase, noun in label_map.items():
                    # 如果 Florence 返回的 label 包含我们构造的 phrase，或者反之
                    if phrase in lbl or lbl in phrase:
                        matched_noun = noun
                        break
                class_names.append(matched_noun)
    else:
        class_names = raw_labels

    class_ids = np.array(list(range(len(class_names))))
    scores = np.ones(len(class_names))
    ...
    # 执行去重 (特别是对于 Phrase Grounding，可能重叠)
    if task_prompt == "<CAPTION_TO_PHRASE_GROUNDING>":
        print(f"[DEBUG] Applying NMS with IoU thr=0.90, IoA thr=0.80...")
        det_res = dedup_det_res_by_mask_iou(det_res, iou_thr=0.90, ioa_thr=0.80)
        # 更新变量以便后续可视化使用
        input_boxes = det_res['boxes']
        masks = det_res['masks']
        class_names = det_res['class_names']
        class_ids = det_res['class_ids']
...
def load_scene_objects(json_path):
    """
    从 scene_recognition.json 中读取 noun / modifiers
    返回：
        - class_list: List[str]
        - object_specs: Dict[str, List[modifiers]]
    """
    ...
    for obj in data.get("scene_objects", []):
        noun = obj["noun"].strip().lower()

        # 关键：规范化 noun（避免 'water bottle' / 'bottle' 不一致）
        noun = noun.replace(" ", "_")

        class_list.append(noun)

        object_specs.setdefault(noun, []).append(obj.get("modifiers", {}))

    # 去重但保持顺序
    class_list = list(dict.fromkeys(class_list))

    return class_list, object_specs
...
if __name__ == "__main__":
    ...
    vlm_output = os.path.expanduser("~/workspace/VLM/scene_recognition.json")
    scene_class_list, scene_object_specs = load_scene_objects(vlm_output)
    ...
    # 使用修改后的 grounded_sam2_detection (支持 Scene Specs 构造 Prompt)
